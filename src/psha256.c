...
master_hash:
        ...
        if ((cpids[child_num] = fork()) != 0) {
            ...
            for (unsigned i = 0; i < HASH_CHUNKS; i++) {
                CHECK_CRYPTO(SHA256_Update(&sctx, fbytes + chunk_num * WORKER_CHUNK_SIZE + i * HASH_CHUNK_SIZE, HASH_CHUNK_SIZE));
            }
    ...
    if (lastlen != 0) {
        ...
        for (unsigned i = 0; i < nparts || (i == nparts && left > 0); i++) {
            CHECK_CRYPTO(SHA256_Update(&master_ctx, fbytes + nchunks * WORKER_CHUNK_SIZE + i * HASH_CHUNK_SIZE, i == nparts ? left : HASH_CHUNK_SIZE));
        }
    }
